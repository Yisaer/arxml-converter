package converter

import (
	"encoding/hex"
	"testing"

	"github.com/stretchr/testify/require"
	"github.com/yisaer/idl-parser/converter"
)

func TestS1APCase(t *testing.T) {
	c, err := NewConverter("../test/s1_ap_test.xml", converter.IDlConverterConfig{IsLittleEndian: false, LengthFieldLength: 4, PaddingLength: 4})
	require.NoError(t, err)
	hexStr := ""
	data, err := hex.DecodeString(hexStr)
	require.NoError(t, err)
	name, v, err := c.Decode(33282, 32769, data)
	require.NoError(t, err)
	require.Equal(t, map[string]interface{}{
		"wiFiApNum": int32(2),
		"wiFiApArray": []interface{}{
			map[string]interface{}{
				"wiFiApName":     "中文 WIFI",
				"wiFiStrength":   int32(12),
				"wiFiEncryption": int32(34),
			},
			map[string]interface{}{
				"wiFiApName":     "English WIFI",
				"wiFiStrength":   int32(56),
				"wiFiEncryption": int32(78),
			},
		},
	}, v)
	require.Equal(t, "reportWiFiApList", name)
}

func TestS1CPCase(t *testing.T) {
	c, err := NewConverter("../test/s1_cp_test.xml", converter.IDlConverterConfig{IsLittleEndian: false, LengthFieldLength: 4, PaddingLength: 4})
	require.NoError(t, err)
	svcId, eventID := SplitUint32ToUint16(2181169157)
	require.Equal(t, uint16(33282), svcId)
	testData := []byte{
		0x00, 0x00, 0x00, 0x08, // 长度字段 (8字节)
		0xEF, 0xBB, 0xBF, // UTF-8 BOM
		0x54, 0x65, 0x73, 0x74, // "Test"
		0x00, // UTF-8 终止符
	}
	k, v, err := c.Decode(svcId, eventID, testData)
	require.NoError(t, err)
	require.Equal(t, "adt_WiFiApName", k)
	require.Equal(t, "Test", v)
}

func SplitUint32ToUint16(num uint32) (high16, low16 uint16) {
	high16 = uint16(num >> 16) // 高16位
	low16 = uint16(num)        // 低16位
	return
}
